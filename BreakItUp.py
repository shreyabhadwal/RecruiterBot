...
def get_final_prompt(context: str, new_question: str) -> str:
    ...
    recruit_prompt = recruit_prompt.format_prompt(context=context,question = new_question).to_string()

    return recruit_prompt


def get_final_answer(recruit_prompt: str) -> str:
    ...
    answer_chain = LLMChain.from_string(llm=answer_llm, template=recruit_prompt)
...
if __name__=="__main__":
    ...
    logging.basicConfig(filename="C:/Users/sbhadwal/SpecChat/breakitup.log",
    ...
    try:
        while True:
            ...
            if chat_history_global != []:
                ...
                logger.debug('New Question Prompt: ' + new_question_prompt)
                logger.debug('\n \n \n')
                logger.debug('New Question: ' + new_question)
                logger.debug('\n \n \n')
            ...
            logger.debug("Final Prompt: "+ final_prompt)
            logger.debug('\n \n \n')

            final_answer = get_final_answer(recruit_prompt=final_prompt)
            logger.debug('Final Answer' + final_answer)
            logger.debug('\n \n \n')
            print(final_answer)
